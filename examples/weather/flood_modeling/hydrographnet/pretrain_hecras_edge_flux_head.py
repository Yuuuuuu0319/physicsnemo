...
def transform_face_residual(
    ...
    scale = target_rms
    if mode in ("per_face_rms", "asinh_per_face_rms", "signed_log1p_per_face_rms"):
    ...
    elif mode in ("target_rms", "asinh_target_rms", "signed_log1p_target_rms"):
    ...
    target = target / scale
    if mode in ("target_rms", "per_face_rms"):
        return prediction - target
    if mode in ("asinh_target_rms", "asinh_per_face_rms"):
        return torch.asinh(prediction) - torch.asinh(target)
    if mode in ("signed_log1p_target_rms", "signed_log1p_per_face_rms"):
...
def compute_face_loss(edge_head, graph, args):
    ...
    return loss, raw_loss, prediction, target
...
def evaluate(edge_head, dataset, args) -> dict[str, float]:
    ...
    with torch.no_grad():
        for graph in dataset:
            ...
            loss, raw_loss, prediction, target = compute_face_loss(edge_head, graph, args)
...
def main() -> None:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for graph in train_loader:
            ...
            loss, raw_loss, _, _ = compute_face_loss(edge_head, graph, args)
            loss.backward()
            optimizer.step()
